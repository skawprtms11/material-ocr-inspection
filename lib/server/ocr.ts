...
// /api/ocr(자재마스터 OCR 등록 검토)와 /api/work-inspection/ocr(모바일 작업검수 OCR 판정)가
// 공유하는 Google Cloud Vision 호출 로직. 기존 /api/ocr의 동작은 그대로 유지한다.
...
function mockResult(originalRoi: RoiRect): OcrRunResult {
  return {
    ok: true,
    provider: "mock",
    extractedText: "",
...
export async function runOcr(params: RunOcrParams): Promise<OcrRunResult> {
  const { image, isCropped, roi, originalRoi, imageWidth = 0, imageHeight = 0 } = params;

  if (process.env.OCR_PROVIDER !== "google-vision" || !getCredentials()) {
    return mockResult(originalRoi);
  }

  try {
    const accessToken = await getAccessToken();
    if (!accessToken) return mockResult(originalRoi);
