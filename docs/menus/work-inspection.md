...
- 처리 순서(mock 모드가 아닐 때):
  ...
  4. `work_inspections`를 `status: "passed"`/`result_summary: "제품검수 수동 합격 (코드/명/LOT 확인)"`/`attempt_count`(+1)/`updated_at`으로 update한 뒤 `inspection_images`에 `image_type: "product"` 행을 insert한다(`metadata.checks`에 체크 결과 저장).
