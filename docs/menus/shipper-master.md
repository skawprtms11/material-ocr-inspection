...
- 조회: `GET /api/shipper-master?department_id=...` (`useFilterStore().departmentId` 변경 시 재조회)
  ...
  - 실DB 모드에서 조회 중 예외가 발생하면 mock 폴백 없이 `502 { error }`를 반환한다. 화면은 `loadError` 상태로 "연결 오류" 배지 + "다시 시도" 버튼을 표시한다.
...
- `loadError`가 true면 목록을 비우지 않되(직전 상태 유지) 안내 바가 "연결 오류"로 바뀌고 "다시 시도" 버튼이 `loadShippers`를 재호출한다.
